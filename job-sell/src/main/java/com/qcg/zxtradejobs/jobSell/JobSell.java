package com.qcg.zxtradejobs.jobSell;
...
public class JobSell extends JobWorker{
    //log4j
    private static Logger log = Logger.getLogger("JobSell.class");
    //Ctor
    public JobSell(){
    }
    public JobSell(String work_name) {
        super(work_name);
    }
    public JobSell(String work_name,ConfigParm config_parm){
    ...
    public static void main( String[] args ){
        ...
        //
/*        String w_n = (worker_name != null) ? worker_name : "job-sell";
        ...
        JobSell jobsell = new JobSell(w_n);
        jobsell.waitWorking(w_ip,w_port,"job-sell");*/
        //

        WareHouse inhouse = new WareHouse();
        inhouse.setObj("recv_tube_name", "sell");
        inhouse.setObj("send_tube_name", "status");
        inhouse.setObj("error_tube_name", "error");
        JobSell jobsell = new JobSell();
        WareHouse outhouse = jobsell.doTask(inhouse);
        log.debug(outhouse);
    }
}
